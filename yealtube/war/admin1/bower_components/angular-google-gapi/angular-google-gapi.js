...
angular.module('angular-google-gapi').factory('GApi', ['$q', 'GClient', 'GData', '$window', '$http',
    function($q, GClient, GData, $window, $http){
        ...
        function runGapi(api, method, params, deferred) {
            ...
            api(params).execute(function (response) {
                if (response.error) {
                	console.log('Call execute REJECT:' + JSON.stringify(response.error));
                    deferred.reject(response);
                } else {
                	console.log('Call execute RESOLVE:' + JSON.stringify(response));
        ...
        function execute(api, method, params, auth) {
            ...
            if (apisLoad.indexOf(api) > -1) {
            	console.log('Call runGapi with api: ' + api);
                runGapi(api, method, params, deferred);
            }
            else
            	console.log('Call registerObserverCallback with api: ' + api);
